	...
	public class StudentsController : Controller
	{
			...
			[FromBody] UpdateStudentRequest request)
		{
			if (await _studentRepository.ExistsAsync(studentId))
			{
				// Update details
				var updatedStudent =
					await _studentRepository.UpdateStudent(studentId, _mapper.Map<DataModels.Student>(request));
		...
		public async Task<IActionResult> UploadImageAsync([FromRoute] Guid studentId, IFormFile profileImage)
		{
			...
			if (profileImage is not { Length: > 0 }) 
				return NotFound();
			
			var extension = Path.GetExtension(profileImage.FileName);
			if (validExtensions.Contains(extension.ToLower()) == false)
				return BadRequest("This is not a valid image format");

			if (await _studentRepository.ExistsAsync(studentId) == false)
				return NotFound();
